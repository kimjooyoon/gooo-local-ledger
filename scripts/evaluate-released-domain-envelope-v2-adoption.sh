...
jq -S -n \
  ...
  def fact($id):
    ...
    elif $id=="PRODUCT_PROJECTION" then ($f.bundle.product_owned_projection==1 and $f.bundle.product_owned_projection_total==1 and $f.projection_inputs.local_projector_identity==true and $f.projection_inputs.envelope_semantic_authority_observed==true and $f.projection_inputs.envelope_semantic_authority=="INTERCHANGE_SPECIFICATION" and $f.projection_inputs.projection_implementation_owner=="LOCAL_LEDGER" and $l.authority.envelope_semantic_authority=="INTERCHANGE_SPECIFICATION" and $l.authority.projection_implementation_owner=="LOCAL_LEDGER" and $f.projection_inputs.core_activities==$f.meta.activities_total and $f.projection_inputs.graph_activities==$f.meta.activities_total and $f.projection_inputs.relation_count==$f.source_artifacts.project_relation_count and $f.projection_inputs.evidence_count==$f.source_artifacts.project_evidence_count and $f.projection_inputs.resolution_count==$f.source_artifacts.project_resolution_count and $f.projection_inputs.conformer_decision=="CONFORMANT" and $f.projection_inputs.conformer_relations==expected("typed_relations") and $f.projection_inputs.conformer_evidence==expected("typed_relations") and $f.projection_inputs.conformer_resolutions==expected("typed_relations"))
  ...
  ([$cells[]|select(.state!="CLOSED")][0]//null) as $first |
  {
    ...
    authority:{
      envelope_semantic_authority:$f.projection_inputs.envelope_semantic_authority,projection_implementation_owner:$f.projection_inputs.projection_implementation_owner,envelope_format_owner:"INTERCHANGE_SPECIFICATION",
